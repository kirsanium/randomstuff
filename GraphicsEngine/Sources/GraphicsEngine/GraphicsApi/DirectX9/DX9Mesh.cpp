...
HRESULT DX9Mesh::CreateVertexBuffer(void * p_gVertices, UINT vertexArraySize, int vertexCount, DWORD fvf)
{
	...
	if( FAILED( m_pDevice->CreateVertexBuffer( vertexArraySize, 0, m_fvf, D3DPOOL_MANAGED, &m_pVertexBuffer, nullptr ) ) )
	{
	    return E_FAIL;
	}
...
HRESULT	DX9Mesh::CreateIndexBuffer(void * p_gIndices, UINT size, int indexCount, D3DPRIMITIVETYPE primitiveType)
{
	...
	if( FAILED( m_pDevice->CreateIndexBuffer( size, 0, D3DFMT_INDEX32, D3DPOOL_MANAGED, &m_pIndexBuffer, nullptr ) ) )
	{
	    return E_FAIL;
	}
