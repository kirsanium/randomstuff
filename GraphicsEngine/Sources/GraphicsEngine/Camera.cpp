...
{
	if (nullptr == pObject || !pObject->IsActive()) return;

	Material *	pMaterial	= (nullptr != pSpecialMaterial) ? pSpecialMaterial : pObject->m_pMaterial;
	Mesh *		pMesh		= pObject->m_pMesh;
	if ((nullptr == pMaterial) || (nullptr == pMesh)) return;
