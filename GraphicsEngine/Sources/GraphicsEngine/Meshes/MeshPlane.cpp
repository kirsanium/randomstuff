...
MeshPlane::MeshPlane(int levelCount)
{
...
void MeshPlane::Deinit()
{
	Mesh::Deinit();
}

void MeshPlane::InitCubeSimple()
{
...
void MeshPlaneSetVertex(int idx, int i, int iMax, int j, int jMax, const Vector3 & bottomLeft, const Vector3 & right, const Vector3 & up,
						std::vector<Vector3> & v, std::vector<Vector3> & n, std::vector<Vector4> & c, const Vector4 & color, std::vector<Vector3> & uv0)
{
	...
	c[idx] = color;
	uv0[idx] = Vector3(1.0f * i / iMax, 1.0f * j / jMax, 0);
}

void MeshPlaneFillPlane(const Vector3 & bottomLeft, const Vector3 & right, const Vector3 & up, int m_levelCount, 
			   int vertexInRowCount, int indexInRowCount, 
			   std::vector<Vector3> & vertices, std::vector<Vector3> & normals, std::vector<Vector4> & colors, std::vector<Vector3> & uv0,
			   std::vector<int> & indices, int planeIndex)
{
	...
	for (int j = 0; j < jMax + 1; ++j)
	{
		for (int i = 0; i < iMax + 1; ++i)
		{
			...
			MeshPlaneSetVertex(idx, i, iMax, j, jMax, bottomLeft, right, up, vertices, normals, colors, color, uv0);
		}		
	}
...
void MeshPlane::InitCubeDetailedStrip()
{
	...
	MeshPlaneFillPlane(Vector3(-0.5f, -0.5f, -0.5f), Vector3( 1.0f, 0.0f, 0.0f), Vector3( 0.0f, 1.0f, 0.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 0);

	MeshPlaneFillPlane(Vector3( 0.5f, -0.5f, -0.5f), Vector3( 0.0f, 0.0f, 1.0f), Vector3( 0.0f, 1.0f, 0.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 1);

	MeshPlaneFillPlane(Vector3( 0.5f, -0.5f,  0.5f), Vector3(-1.0f, 0.0f, 0.0f), Vector3( 0.0f, 1.0f, 0.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 2);

	MeshPlaneFillPlane(Vector3(-0.5f, -0.5f,  0.5f), Vector3( 0.0f, 0.0f,-1.0f), Vector3( 0.0f, 1.0f, 0.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 3);

	MeshPlaneFillPlane(Vector3(-0.5f,  0.5f, -0.5f), Vector3( 1.0f, 0.0f, 0.0f), Vector3( 0.0f, 0.0f, 1.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 4);

	MeshPlaneFillPlane(Vector3( 0.5f, -0.5f,  0.5f), Vector3( 0.0f, 0.0f,-1.0f), Vector3(-1.0f, 0.0f, 0.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 5);
...
void FillPlane2(const Vector3 & bottomLeft, const Vector3 & right, const Vector3 & up, int m_levelCount, 
			   int vertexInRowCount, int indexInRowCount, 
			   std::vector<Vector3> & vertices, std::vector<Vector3> & normals, std::vector<Vector4> & colors, std::vector<Vector3> & uv0,
			   std::vector<int> & indices, int planeIndex)
{
	...
	for (int j = 0, k = 0; j < jMax; ++j)
	{
		for (int i = 0; i < iMax; ++i)
		{
			...
			MeshPlaneSetVertex(idx0, i + 0, iMax, j + 0, jMax, bottomLeft, right, up, vertices, normals, colors, color, uv0);

			int idx1 = vertexShift + (k + 1);
			MeshPlaneSetVertex(idx1, i + 0, iMax, j + 1, jMax, bottomLeft, right, up, vertices, normals, colors, color, uv0);

			int idx2 = vertexShift + (k + 2);
			MeshPlaneSetVertex(idx2, i + 1, iMax, j + 1, jMax, bottomLeft, right, up, vertices, normals, colors, color, uv0);

			int idx3 = vertexShift + (k + 3);
			MeshPlaneSetVertex(idx3, i + 1, iMax, j + 0, jMax, bottomLeft, right, up, vertices, normals, colors, color, uv0);
...
void MeshPlane::InitCubeDetailedList()
{
	...
	int vertexCount			= vertexInPlaneCount;				// (quad count on side)
	...
	int indexCount			= indexInPlaneCount;				// (index count on side)

	std::vector<int> indices;
	
	indices.resize(indexCount);
	
	FillPlane2(Vector3(-0.5f,  0.0f, -0.5f), Vector3( 1.0f, 0.0f, 0.0f), Vector3( 0.0f, 0.0f, 1.0f),
						  m_levelCount, vertexInRowCount, indexInRowCount, vertices, normals, colors, uv0, indices, 0);
